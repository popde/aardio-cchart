...
// 以下函数来自CChartWnd
	...
	// 重绘方式选择。
	// bDir为false时，向窗口发送重绘消息。线程安全
	// bDir为true时，直接调用OnDraw画图。效率高
	// 内部默认状态为false。单线程时，可以考虑设置为true
	CChart_API void	SetDirectReDraw(bool bDir);
...
//CChart_API  以下函数来自CChart
	...
	// 打印图像
	CChart_API void		PrintImage(HWND hWnd, bool bStretch);
	// 打印图像
	CChart_API void		PrintImage(HDC hDC, bool bStretch);
	...
	// 测试图像是否双缓冲
	CChart_API bool		IsDoubleBuffer();
	// 设置图像是否双缓冲
	CChart_API void		SetDoubleBuffer(bool bDb);
	...
	// 设置是否响应数据
	CChart_API void		SetReactData(bool react);
	...
	// 对预置窗口发重绘消息，主要用于多线程，以保证线程安全
	CChart_API void		Invalidate(BOOL bErase = TRUE);
	...
	// 初始化等高线时自定义动作，因为有时可能比较慢，可以用来显示进度条
	CChart_API void		SetContourUserAction(void (*fcnUserAction)());
	// 等高线初始化的动作数
	CChart_API int		GetContourInitSteps();
	...
	// 设置自定义工具提示函数
	CChart_API void		SetUserFcnTooltip(TCHAR	*(*fcnUserTooltip)( void *plot, RECT clientRect, RECT plotRect, POINT point, double *data, double *xRange, double *yRange, int nPointIndex, int nDataIndex, void *pPara), void *pPara);
	...
	// 改变曲线条数
	// 如果nCount比现有曲线条数多，则增加空曲线
	// 如果nCount比现有曲线条数少，则删除部分曲线，先删后添加的。
	CChart_API void		ResizeCurves(int nCount, int nPlotIndex=0);
	// 添加曲线，前三个参数是曲线数据
	...
	// nDataIndex表示曲线的序号,
	// nDataIndex如果等于当前曲线的条数，则新增一条曲线，如果小于0或者大于当前曲线条数，则添加失败
	CChart_API int			AddPoints2D(double *pX, double *pY, int nLen, int nDataIndex=0, int nPlotIndex=0);
	...
	// 获得图中曲线最多的数据点数
	CChart_API int			GetMaxDataPointsCount(int nPlotIndex=0);
	// 获取数据点x
	...
	// 替换曲线上第一个数据点
	CChart_API int			ReplaceFirstPoint(double x, double y, int nDataIndex, int nPlotIndex=0);
	// 替换曲线上最后一个数据点
	CChart_API int			ReplaceLastPoint(double x, double y, int nDataIndex, int nPlotIndex=0);
	// 滑动X数据
	...
	// 设置在柱顶显示数值
	CChart_API void		SetShowValOnTop(bool show);
	...
	// 允许通过鼠标点击添加等高线
	CChart_API void		SetMouseAddContour(bool bAdd);
	// 允许云图中有洞
	CChart_API void		SetEnableContourHole(bool enable);
	// 设置洞的标识，用于场函数
	CChart_API void		SetContourHoleFlag(bool flag);
	// 设置云图中洞的颜色
	CChart_API void		SetContourHoleColor(COLORREF cr);
	// 设置等高线是否显示，只用于等高云图。
	// 如果等高线不显示，则等高云图基本等同于云图，不同之处在于等高云图可以手动添加曲线
	CChart_API void		SetContourLineShow(bool show);
	
	// 设置云图的起始颜色
	CChart_API void		SetMapColor1(COLORREF cr);
	// 设置云图的最终颜色
	CChart_API void		SetMapColor2(COLORREF cr);
	...
	// 设置热图的宽度和高度
	CChart_API void		SetHeatDimension(int width, int height);
	// 设置热图的数据
	CChart_API void		SetHeatData(double val, int iRow, int iCol);
	// 设置热图、云图的数值范围
	CChart_API void		SetMapRange(double low, double high);
	// 设置自动计算热图、云图的数值范围
	CChart_API void		SetAutoMapRange(bool bAt);
	...
	// 设置曲线数据点单独标记一个点
	CChart_API void		SetMarkSinglePoint(bool bMark, int nDataIndex, int nPlotIndex=0);
	// 设置曲线数据点单独标记一个点
	CChart_API void		SetMarkPointIndex(int nPointIndex, int nDataIndex, int nPlotIndex=0);
	...
	// 设置数据线是否显示
	// 注意，和SetDataVisible不同，这里除了线，还可以显示别的，如Marker
	CChart_API void		SetDataLineShow(bool bShow, int nDataIndex, int nPlotIndex=0);
	...
	// 获得选择曲线的序号，目前只适用于折线图
	CChart_API int			GetIndexOfSelected();
	...
	// 添加曲线数据点的注释
	CChart_API void		AddDataPointComment(TCHAR *comment, int nPointIndex, int nDataIndex, int nPlotIndex=0);
	// 设置曲线最大数据点数，添加点的时候，总数超出的话，自动移除第一个点
	CChart_API void		SetDataMaxPoints(int nMaxPoints, int nDataIndex, int nPlotIndex=0);
	// 设置曲线自动圆滑
	CChart_API void		SetDataAutoSlick(bool bSlick, int nDataIndex, int nPlotIndex=0);
	// 设置曲线线段隐藏模式
	CChart_API void		SetDataSegHideMode(bool hide, int nDataIndex, int nPlotIndex=0);
	// 添加曲线的隐藏线段
	// nPointIndex为隐藏线段的起点
	// 例如，如需要隐藏点3到点10之间的所有线段，那这个函数就需要调用7次，每次的nPointIndex从3到9
	CChart_API void		AddDataSegHidden(int nPointIndex, int nDataIndex, int nPlotIndex=0);
	// 清除所有隐藏线段
	CChart_API void		ClrDataSegHidden(int nDataIndex, int nPlotIndex=0);
	// 清除最先的隐藏线段
	CChart_API void		ClrFirstSegHidden(int nDataIndex, int nPlotIndex=0);
	// 清除最后的隐藏线段
	CChart_API void		ClrLastSegHidden(int nDataIndex, int nPlotIndex=0);
	...
	// 删除所有坐标轴
	CChart_API void		DeleteAxes();
	// 删除子图所有坐标轴
	CChart_API void		DeleteAxes(int nPlotIndex);
	// 删除坐标轴
	CChart_API void		DeleteAxisByLocation(int location);
	// 删除子图坐标轴
	CChart_API void		DeleteAxisByLocation(int location, int nPlotIndex);
	// 设置子图坐标轴标题
	...
	// 设置子图坐标轴只显示两端标签
	CChart_API void		SetAxisLabelAtEnd(bool bEnd, int location, int nPlotIndex);
	// 设置坐标轴只显示两端标签
	CChart_API void		SetAxisLabelAtEnd(bool bEnd, int location);
	// 设置子图坐标轴两端标签内收
	CChart_API void		SetAxisLabelWithdraw(bool bWithdraw, int location, int nPlotIndex);
	// 设置坐标轴两端标签内收
	CChart_API void		SetAxisLabelWithdraw(bool bWithdraw, int location);
	// 设置子图坐标轴两端标签内收长度
	CChart_API void		SetAxisLabelWithdrawLength(int nLen, int location, int nPlotIndex);
	// 设置坐标轴两端标签内收长度
	CChart_API void		SetAxisLabelWithdrawLength(int nLen, int location);
	// 设置子图坐标轴标题位置
	...
	// 设置子图时间坐标轴混合模式。设置此参数后，数据只需按秒添加，可以带任意小数。加上基准时间，就是坐标轴显示的时间。
	CChart_API void		SetAxisTimeMixMode(bool bMix, int location, int nPlotIndex);
	// 设置时间坐标轴混合模式。设置此参数后，数据只需按秒添加，可以带任意小数。加上基准时间，就是坐标轴显示的时间。
	CChart_API void		SetAxisTimeMixMode(bool bMix, int location);
	// 设置子图时间坐标轴基准
	CChart_API void		SetAxisTimeBase(TCHAR *strTime, TCHAR *strFmt, int location, int nPlotIndex);
	// 设置时间坐标轴基准
	CChart_API void		SetAxisTimeBase(TCHAR *strTime, TCHAR *strFmt, int location);
	// 设置子图显示颜色棒
	...
	// 设置子图横轴标签旋转90度坐标轴
	CChart_API void		SetXLabelRotate(bool bRot, int location, int nPlotIndex);
	// 设置横轴标签旋转90度坐标轴
	CChart_API void		SetXLabelRotate(bool bRot, int location);
	// 设置子图横轴标签旋转90度方向，0左旋，1右旋
	CChart_API void		SetXLabelRotateDirect(int direct, int location, int nPlotIndex);
	// 设置横轴标签旋转90度方向，0左旋，1右旋
	CChart_API void		SetXLabelRotateDirect(int direct, int location);
	...
	// 子图X坐标范围根据间隔设置
	CChart_API void		SetXRangeByInterval(bool bBy, int nPlotIndex);
	// X坐标范围根据间隔设置
	CChart_API void		SetXRangeByInterval(bool bBy);
	// 子图Y坐标范围根据间隔设置
	CChart_API void		SetYRangeByInterval(bool bBy, int nPlotIndex);
	// Y坐标范围根据间隔设置
	CChart_API void		SetYRangeByInterval(bool bBy);
	// 子图X坐标刻度间隔
	CChart_API void		SetXTicksInterval(double interval, int nPlotIndex);
	// X坐标刻度间隔
	CChart_API void		SetXTicksInterval(double interval);
	// 子图Y坐标刻度间隔
	CChart_API void		SetYTicksInterval(double interval, int nPlotIndex);
	// Y坐标刻度间隔
	CChart_API void		SetYTicksInterval(double interval);
	// 设置子图X坐标轴浮动标记
	...
	// 设置坐标轴范围自动计算
	CChart_API void		SetAutoRange(bool bAuto);
	CChart_API void		SetAutoRange(bool bAuto, int nPlotIndex);
	...
	// 设置背景图像
	CChart_API void		SetBkgndImage(TCHAR *strImgName);
	...
	// 设置图像左预留空白，用于最左边标签显示
	CChart_API void		SetReservedMarginLeft(int margin);
	// 设置分裂视图左预留空白，用于最左边标签显示
	CChart_API void		SetReservedMarginLeft(int margin, int nPlotIndex);
	// 设置图像右预留空白，用于最右边标签显示
	CChart_API void		SetReservedMarginRight(int margin);
	// 设置分裂视图右预留空白，用于最右边标签显示
	CChart_API void		SetReservedMarginRight(int margin, int nPlotIndex);
	// 设置图像上预留空白，用于最上边标签显示
	CChart_API void		SetReservedMarginTop(int margin);
	// 设置分裂视图上预留空白，用于最上边标签显示
	CChart_API void		SetReservedMarginTop(int margin, int nPlotIndex);
	// 设置图像下预留空白，用于最下边标签显示
	CChart_API void		SetReservedMarginBottom(int margin);
	// 设置分裂视图下预留空白，用于最下边标签显示
	CChart_API void		SetReservedMarginBottom(int margin, int nPlotIndex);
	...
	// 设置图例显示水平，0:显示所有曲线，1:隐藏visible为false的曲线，2:隐藏visible为false的曲线和空曲线
	CChart_API void		SetLegendLevel(int nLevel, int nPlotIndex=0);
	...
	// 根据数据值添加备注
	CChart_API int		AddComment(TCHAR *comment, double x, double y, int nPlotIndex=0);
	// 设置上一个函数参数中，x,y是否是相对值。相对值的定义，就是本身值减去范围最小值
	CChart_API void		SetCommentPosRelative(bool relative, int nCommentIndex, int nPlotIndex=0);
	// 设置备注的位置，用于在AddComment后修改
	CChart_API void		SetCommentPos(int nCommentIndex, double x, double y, int nPlotIndex=0);
	// 底层函数，获取子图的绘图区域
	...
	// 底层函数，设置绘图区域固定
	CChart_API void		SetPlotRectFixed(bool bFixLeft, bool bFixRight, bool bFixBottom, bool bFixTop, int nPlotIndex=0);
	// 底层函数，设置固定的绘图区域
	CChart_API void		SetFixedPlotRect(long nFixLeft, long nFixRight, long nFixBottom, long nFixTop, int nPlotIndex=0);
	// 底层函数，获得坐标轴大小
	CChart_API SIZE		GetAxisSize(int location, int nPlotIndex=0);
	// 底层函数，设置分裂视图水平分隔条位置
	CChart_API void		SetRowRatio(double ratio, int nSplitIndex, int nPlotIndex=0);
	// 底层函数，设置分裂视图竖直分隔条位置
	CChart_API void		SetColRatio(double ratio, int nSplitIndex, int nPlotIndex=0);
